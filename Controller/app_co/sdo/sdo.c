...
bool push_data_into_queue_to_send(void (*method)(void),void (*response)(void),
    ...
    SDO_mailbox.sdo_send_msg[SDO_mailbox.msg_waiting].response_fucntion_abort   = response_fucntion_abort;
...
void send_sdo_process(sdo_send_mailbox *mailbox){
    ...
    case CO_SDO_RT_abort:
        CO_SDO_reset_status(p_sdo);
        if(sdo_sending_msg.response_fucntion_abort != NULL){
            sdo_sending_msg.response_fucntion_abort();
        }
        break;
    }
}
