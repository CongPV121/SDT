...
{
    this->timer_bootMaster      = new QTimer();
    this->timer_1ms             = new QTimer();
    this->timer_testing_process = new QTimer();
    this->timer_Notification    = new QTimer();

    connect(this->timer_bootMaster, &QTimer::timeout,this, &thread::timeout_timer_bootMaster_processing);
...
void thread::timeout_timer_bootMaster_processing(){
    sys_timestamp += 1;
